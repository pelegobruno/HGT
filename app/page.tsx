...
import { Heart, Activity, Droplet, PlusCircle, X, Bell, Camera, Home, AlertCircle } from 'lucide-react';
...
export default function EloVitalApp() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans antialiased text-gray-800">
      ...
      <nav className="bg-white border-b border-gray-200 px-6 py-4 sticky top-0 z-30 shadow-sm">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-3 text-teal-700">
            <div className="bg-teal-500 p-2 rounded-xl flex items-center justify-center">
              <Home className="w-6 h-6 text-white" />
            </div>
            <span className="text-xl font-extrabold tracking-tight">EloVital<span className="text-gray-400 font-light"> Família</span></span>
          </div>
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="relative group cursor-pointer" onClick={() => inputFotoRef.current?.click()}>
              ...
              <img 
                src={fotoPerfil} 
                alt="Perfil" 
                className="w-12 h-12 rounded-full border-2 border-white shadow-md object-cover group-hover:opacity-75 transition-all"
              />
      ...
      {precisaLembrete && (
        <div className="bg-amber-50 border-l-4 border-amber-500 p-4 mx-4 sm:mx-6 lg:mx-auto max-w-7xl mt-6 rounded-r-xl shadow-sm flex items-center gap-4">
          <AlertCircle className="w-6 h-6 text-amber-500 flex-shrink-0" />
          <p className="text-amber-800 font-medium">Lembrete diário: Olá! Você ainda não registrou os sinais vitais de hoje. Que tal fazer isso agora?</p>
        </div>
      )}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {abaAtiva === 'painel' && (
          <div className="animate-in fade-in duration-300">
            ...
            {/* O BANNER MELHORADO AQUI */}
            <div className="relative overflow-hidden bg-gradient-to-br from-teal-500 via-teal-600 to-teal-900 rounded-3xl p-8 md:p-10 text-white shadow-2xl flex flex-col md:flex-row justify-between items-center mb-10 border border-teal-400/20">
              ...
              <div className="relative z-10 mb-6 md:mb-0">
                <h1 className="text-3xl md:text-4xl font-extrabold mb-2 tracking-tight">Monitoramento Doméstico</h1>
                <p className="text-teal-50 text-lg max-w-lg font-medium opacity-90">
                  {precisaLembrete ? "Nuvem conectada. Vamos fazer a primeira medição do dia?" : "Sinais vitais salvos na nuvem e atualizados com sucesso!"}
                </p>
              </div>
              
              <button 
                onClick={() => setModalAberto(true)}
                className="relative z-10 bg-white text-teal-700 hover:bg-gray-50 px-8 py-4 rounded-full font-bold flex items-center gap-3 transition-all duration-300 hover:shadow-xl hover:scale-105 active:scale-95 border-2 border-transparent hover:border-teal-100"
              >
                <PlusCircle className="w-6 h-6 text-teal-500" />
                <span className="text-lg">Nova Medição</span>
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <div className="bg-red-50 p-3 rounded-xl"><Droplet className="w-6 h-6 text-red-500" /></div>
                    {statusGlicemia && (
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${statusGlicemia.cor}`}>
                        {statusGlicemia.texto}
                      </span>
                    )}
                  </div>
                  <p className="text-gray-500 font-medium mb-1">Glicemia (HGT)</p>
                </div>
                <div className="flex items-baseline gap-2 mt-2">
                  <h3 className="text-3xl font-extrabold text-gray-900">{glicemiaAtual}</h3>
                  {glicemiaAtual !== "--" && <span className="text-gray-400 font-medium text-sm">mg/dL</span>}
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <div className="bg-emerald-50 p-3 rounded-xl"><Heart className="w-6 h-6 text-emerald-500" /></div>
                    {statusPressao && (
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${statusPressao.cor}`}>
                        {statusPressao.texto}
                      </span>
                    )}
                  </div>
                  <p className="text-gray-500 font-medium mb-1">Pressão Arterial</p>
                </div>
                <div className="flex items-baseline gap-2 mt-2">
                  <h3 className="text-3xl font-extrabold text-gray-900">{pressaoAtual}</h3>
                  {pressaoAtual !== "--" && <span className="text-gray-400 font-medium text-sm">mmHg</span>}
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <div className="bg-blue-50 p-3 rounded-xl"><Activity className="w-6 h-6 text-blue-500" /></div>
                    {statusOxi && (
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${statusOxi.cor}`}>
                        {statusOxi.texto}
                      </span>
                    )}
                  </div>
                  <p className="text-gray-500 font-medium mb-1">Oxigênio (SpO2)</p>
                </div>
                <div className="flex items-baseline gap-2 mt-2">
                  <h3 className="text-3xl font-extrabold text-gray-900">{oximetriaAtual}</h3>
                  {oximetriaAtual !== "--" && <span className="text-gray-400 font-medium text-sm">%</span>}
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <div className="bg-rose-50 p-3 rounded-xl"><Activity className="w-6 h-6 text-rose-500" /></div>
                    {statusBpm && (
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${statusBpm.cor}`}>
                        {statusBpm.texto}
                      </span>
                    )}
                  </div>
                  <p className="text-gray-500 font-medium mb-1">Batimentos Cardíacos</p>
                </div>
                <div className="flex items-baseline gap-2 mt-2">
                  <h3 className="text-3xl font-extrabold text-gray-900">{batimentosAtual}</h3>
                  {batimentosAtual !== "--" && <span className="text-gray-400 font-medium text-sm">BPM</span>}
                </div>
              </div>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
              <div className="mb-8">
                <h2 className="text-2xl font-bold text-gray-900">Evolução Glicêmica (HGT)</h2>
                <p className="text-gray-500 mt-1">Sincronizado com a nuvem do Firebase</p>
              </div>
              ...
              <div className="h-96 w-full">
                {dadosGrafico.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-gray-400 border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50">
                    <Droplet className="w-12 h-12 mb-4 text-gray-300" />
                    <p className="text-lg font-bold">Sem dados de HGT no banco</p>
                    <p className="text-sm">Preencha a Glicemia para visualizar a curva.</p>
                  </div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={dadosGrafico} margin={{ top: 10, right: 10, bottom: 10, left: -20 }}>
                      ...
                      <Line type="monotone" dataKey="hgtNumero" name="Glicemia" stroke="#0D9488" strokeWidth={5} dot={{ r: 6, fill: '#fff', strokeWidth: 3, stroke: '#0D9488' }} />
                    </LineChart>
                  </ResponsiveContainer>
                )}
              </div>
          ...
          <div className="animate-in fade-in duration-300">
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
              <div className="flex justify-between items-center mb-8">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">Histórico da Nuvem</h2>
                </div>
              </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b-2 border-gray-100 text-gray-500 text-sm uppercase tracking-wider">
                        ...
                        <th className="pb-4 px-4">HGT</th>
                        <th className="pb-4 px-4">Pressão</th>
                        <th className="pb-4 px-4">SpO2</th>
                        <th className="pb-4 px-4">BPM</th>
                      </tr>
                    </thead>
                    <tbody className="text-gray-700">
                      {listaMedicoes.slice().reverse().map((medicao, index) => (
                        <tr key={medicao.id || index} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                          <td className="py-4 px-4 font-medium">{medicao.hora}</td>
                          <td className="py-4 px-4">{medicao.hgtTexto} {medicao.hgtTexto !== "--" && "mg/dL"}</td>
                          <td className="py-4 px-4">{medicao.pressao}</td>
                          <td className="py-4 px-4">{medicao.oximetria} {medicao.oximetria !== "--" && "%"}</td>
                          <td className="py-4 px-4">{medicao.batimentos}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      {modalAberto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl relative z-10 animate-in fade-in zoom-in duration-200">
            ...
            <div className="space-y-6 mb-8 max-h-[60vh] overflow-y-auto pr-2">
              ...
              <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                <label className="flex items-center gap-2 text-gray-700 font-bold mb-3">
                  <Droplet className="w-5 h-5 text-red-500"/> Glicemia (HGT)
                </label>
                <div className="flex items-center gap-3">
                  <input type="number" value={formHGT} onChange={(e) => setFormHGT(e.target.value)} placeholder="000"
                    className="w-full text-2xl font-black text-center text-gray-900 bg-white border-2 border-gray-200 rounded-xl py-3 focus:border-teal-500 focus:outline-none"
                  />
                  <span className="text-gray-400 font-medium">mg/dL</span>
                </div>
              </div>
              
              <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                <label className="flex items-center gap-2 text-gray-700 font-bold mb-3">
                  <Heart className="w-5 h-5 text-emerald-500"/> Pressão Arterial
                </label>
              ...
              <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100 flex gap-4">
                <div className="w-1/2">
                  <label className="flex items-center gap-2 text-gray-700 font-bold mb-3 text-sm">
            ...
            <button 
              ...
              className="w-full bg-teal-600 hover:bg-teal-700 text-white text-xl font-bold py-4 rounded-xl shadow-lg active:scale-95 transition-all"
            >
              Confirmar Dados
            </button>
          </div>
